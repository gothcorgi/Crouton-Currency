...
public class Notes : MonoBehaviour
{
    ...
    public GameObject myGameObject; //we can reference any object in our scene. every items in the hierarchy is considered a gameobject 
    public Notes mynotescript; //we can reference scripts as well as long as they are public 
    public Rigidbody2D myRB2D; //we use rigidbodies to make gameobjects affected by unity's physics engine 2D and 3D are independent 
    ...
    void Start()
    {
        ...
        //components in the inspector can be enabled or disabled 
        myboxcollider.enabled = true; //small check boxes inside the game object 
        myGameObject.SetActive(true); //the whole game object 
        //anything that we want to happen when the game starts goes here. it is only called once when the game first starts 
        ...
        //generally we only use FindObjectofType when there only are singular object of that type 
        ...
        //rotation is a bit different we have to use Quarternion and EULER (oiler) 
        transform.rotation = Quarternion.Euler(0, 0, 0);
        transform.rotation = Quarternion.identity; //this is the current rotation of the object 
    }

    // Update is called once per frame
    void Update()
    {
        //inside update we call things that we want to change in realtime as the player is playing 
        //player controls, enemy controlls, points, health etc all ahppen inside the update loop 
...
//} 
        ...
        //the double equal sign is needed for this nto work as its checks to see IF the statement 
        //is true or fase "yesorno = true" automatically sets the bool to be true 
        if(number >=10)
        {
            //we do something 
        }
        ...
        if(Input.GetAxis("Horizontal")>0)
        {
            ...
            //so that jump doesn't get affected 
        }
    ...
    private void FixedUpdate()
    {
        ...
        //FIXED UPDATE updates at a set interval which means all machines run this code at the same speed. we put physics code inside fixed update 
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        //it could and enemy, an animation, music really anything we want 
    ...
    {
        //same as trigger except we don't pass through the collider and the collider is not marked as a trigger. this will trigger events when we bump into a collider 
    }
}
